...
def lookup_expression(
    ...
    *,
    ...
    candidate_files = rg_candidate_files(corpus_dir, expression, ignore_case=ignore_case)
...
def select_text_files(
    ...
    *,
    ...
    files = [
        ...
        if path_matches_filters(path, corpus_dir, venues=venues, year=year, year_from=year_from, year_to=year_to)
    ]
...
def rg_candidate_files(corpus_dir: Path, expression: str, *, ignore_case: bool) -> set[Path] | None:
    ...
    command.extend([expression, str(corpus_dir)])
...
def load_manifest_metadata(db_path: Path) -> dict[str, PaperMeta]:
    ...
    for venue, year, title, doi, text_path in rows:
        ...
        metadata[str(Path(text_path))] = meta
        metadata[Path(text_path).name] = meta
    return metadata


def metadata_for_path(path: Path, metadata: dict[str, PaperMeta]) -> PaperMeta:
    return metadata.get(str(path)) or metadata.get(path.name) or PaperMeta()
